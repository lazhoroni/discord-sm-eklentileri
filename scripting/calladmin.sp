...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_rapor", Command_Call);
	...
	g_hSpamTime               = AutoExecConfig_CreateConVar("sm_calladmin_spamtime", "30", "An user must wait this many seconds after a report before he can issue a new one", FCVAR_NONE, true, 0.0);
	g_hReportTime             = AutoExecConfig_CreateConVar("sm_calladmin_reporttime", "60", "An user cannot be reported again for this many seconds", FCVAR_NONE, true, 0.0);
...
public Action Timer_Advert(Handle timer)
{
	if (g_iCurrentTrackers > 0)
	{
		...
		{
			PrintToChatAll("\x04[Rapor]\x03 %t", "CallAdmin_AdvertMessageSingular", g_iCurrentTrackers);
		}
		else
		{
			PrintToChatAll("\x04[Rapor]\x03 %t", "CallAdmin_AdvertMessagePlural", g_iCurrentTrackers);
		}
	}
...
public Action Command_Call(int client, int argc)
{
	...
	{
		ReplyToCommand(client, "\x04[Rapor]\x03 %t", "CallAdmin_CommandNotAllowed", g_iSpamTime - ( GetTime() - g_iLastReport[client] ));
...
public Action Command_HandleCall(int client, int argc)
{
	...
	{
		ReplyToCommand(client, "\x04[Rapor]\x03 %t", "CallAdmin_NoAdmin");
	...
	if (argc != 1)
	{
		...
		ReplyToCommand(client, "\x04[Rapor]\x03 %t: %s <id>", "CallAdmin_WrongNumberOfArguments", cmdName);
	...
	{
		ReplyToCommand(client, "\x04[Rapor]\x03 %t", "CallAdmin_WrongReportID");
	...
	if (reportIndex == -1)
	{
		ReplyToCommand(client, "\x04[Rapor]\x03 %t", "CallAdmin_ReportAlreadyHandled");
...
public int MenuHandler_ConfirmCall(Menu menu, MenuAction action, int client, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		{
			PrintToChat(client, "\x04[Rapor]\x03 %t", "CallAdmin_CallAborted");
		}
	}
...
bool PreReportCheck(int client, int target)
{
	...
	if (!IsClientValid(target))
	{
		PrintToChat(client, "\x04[Rapor]\x03 %t", "CallAdmin_NotInGame");
	...
	{
		PrintToChat(client, "\x04[Rapor]\x03 %t", "CallAdmin_AlreadyReported");
...
bool ReportPlayer(int client, int target, char[] sReason)
{
	...
	{
		PrintToChat(client, "\x04[Rapor]\x03 %t", "CallAdmin_IngameAdminNotified");
	...
	if (g_bPublicMessage)
	{
		PrintToChatAll("\x04[Rapor]\x03 %t", "CallAdmin_HasReported", client, target, sReason);
	}
	else
	{
		PrintToChat(client, "\x04[Rapor]\x03 %t", "CallAdmin_YouHaveReported", target, sReason);
	}
...
void ShowClientSelectMenu(int client)
{
	...
	if (menu.ItemCount < 1)
	{
		PrintToChat(client, "\x04[Rapor]\x03 %t", "CallAdmin_NoPlayers");
	}
	else
	{
		menu.Display(client, 30);
...
public int MenuHandler_BanReason(Menu menu, MenuAction action, int client, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		if (StrEqual("Own reason", sInfo))
		{
			...
			PrintToChat(client, "\x04[Rapor]\x03 %t", "CallAdmin_TypeOwnReason");
			return;
		}
...
public Action ChatListener(int client, const char[] command, int argc)
{
	...
	if (g_bAwaitingReason[client] && !IsChatTrigger())
	{
		...
		// Has aborted
		if (StrEqual(sReason, "!noreport") || StrEqual(sReason, "!raporiptal"))
		{
			PrintToChat(client, "\x04[Rapor]\x03 %t", "CallAdmin_CallAborted");
		...
		{
			g_bAwaitingReason[client] = true;
			PrintToChat(client, "\x04[Rapor]\x03 %t", "CallAdmin_OwnReasonTooShort");
...
stock void PrintNotifyMessageToAdmins(int client, int target)
{
	for (int i; i <= MaxClients; i++)
	{
		if (IsClientValid(i) && !IsFakeClient(i) && !IsClientSourceTV(i) && !IsClientReplay(i) && CheckCommandAccess(i, "sm_calladmin_admin", ADMFLAG_BAN, false) && Forward_OnAddToAdminCount(i)) 
		{
			PrintToChat(i, "\x04[Rapor]\x03 %t", "CallAdmin_AdminNotification", client, target, g_sTargetReason[client]);
		}
	}	
}
