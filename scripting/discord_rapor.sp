...
#define REPORT_MSG "{\"username\":\"{BOTNAME}\", \"content\":\"{MENTION}\",\"attachments\": [{\"color\": \"{COLOR}\",\"title\": \"{HOSTNAME} (steam://connect/{SERVER_IP}:{SERVER_PORT}){REFER_ID}\",\"fields\": [{\"title\": \"Neden\",\"value\": \"{REASON}\",\"short\": true},{\"title\": \"Raporlayan\",\"value\": \"{REPORTER_NAME} ({REPORTER_ID})\",\"short\": true},{\"title\": \"Supheli\",\"value\": \"{TARGET_NAME} ({TARGET_ID})\",\"short\": true}]}]}"
#define CLAIM_MSG "{\"username\":\"{BOTNAME}\", \"content\":\"{MENTION} {MSG}\",\"attachments\": [{\"color\": \"{COLOR}\",\"title\": \"{HOSTNAME} (steam://connect/{SERVER_IP}:{SERVER_PORT})\",\"fields\": [{\"title\": \"Admin\",\"value\": \"{ADMIN}\",\"short\": false}]}]}"
...
public void OnPluginStart()
{
	...
	g_cClaimMsg = CreateConVar("discord_calladmin_claimmsg", "Admin rapor ile ilgileniyor.", "Message to send when admin uses the claim command.");
	...
	AutoExecConfig(true, "discord_rapor");
	
	RegAdminCmd("sm_claim", Cmd_Claim, ADMFLAG_BAN);
	RegAdminCmd("sm_geldim", Cmd_Claim, ADMFLAG_BAN);
}
...
public Action Cmd_Claim(int client, int args)
{
	...
	ReplyToCommand(client, "Mesaj gonderildi.");
